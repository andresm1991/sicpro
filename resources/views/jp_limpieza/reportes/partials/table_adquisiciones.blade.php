<div class="report-container">
    ...
    {{-- Bucle principal que itera sobre cada categoría --}}
    @forelse ($reportData as $categoria => $items)

        {{-- Título de la categoría --}}
        <h4 class="mt-4 mb-3 text-primary">{{ $categoria }}</h4>
                    ...
                    <tbody>
                        @foreach ($items as $item)
                            <tr>
                                ...
                                <td>{{ collect($item['detalles'])->pluck('necesidad')->implode(', ') }}</td>
                                <td class="text-right">$ {{ number_format($item['total_general'], 4) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                ...
                @elseif($categoria === 'Contratistas')
                    ...
                    <tfoot class="font-weight-bold">
                        <tr>
                            ...
                            <td class="text-right">$ {{ number_format(collect($items)->sum('monto'), 4) }}</td>
                        </tr>
                    </tfoot>
                @endif
            </table>
    ...
    @if (!empty($reportData))
        <div class="text-right mt-4">
            <h3>Total general: <span class="badge badge-success">$ {{ number_format($grandTotal, 4) }}</span></h3>
        </div>
    @endif
</div>
