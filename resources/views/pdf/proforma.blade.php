        ...
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <td style="width: 50%; text-align: right; vertical-align: top;">
                    ...
                    <p style="margin: 5px 0; font-size: 14px;"><strong>RUC: {{ $empresa[4]->detalle }}</strong></p>
                    <h2
        ...
        <table
            ...
            <thead>
                <tr style="background-color: #f2f2f2;">
                    ...
                    <th style="width: 55%; border: 1px solid #000; padding: 8px; text-align: center;">DESCRIPCIÓN</th>
                    <th style="width: 10%; border: 1px solid #000; padding: 8px; text-align: center;">
                        {{ $tipo == 'adecentamientos' ? 'CANT' : 'AREA M2' }} </th>
                    <th style="width: 15%; border: 1px solid #000; padding: 8px; text-align: center;">UNITARIO</th>
                    <th style="width: 15%; border: 1px solid #000; padding: 8px; text-align: center;">V/ TOTAL</th>
                </tr>
            </thead>
            <tbody>
                <!-- Fila de ejemplo -->
                @forelse ($tipo == 'adecentamientos' ? $proforma->detalleAdecentamientos : $proforma->detallePlanos as $index => $detalle)
                    <tr>
                        ...
                        <td style="border: 1px solid #000; padding: 8px;">{{ strtoupper($detalle->producto->nombre) }}
                        </td>
                        <td style="border: 1px solid #000; padding: 8px;">
                            {{ $tipo == 'adecentamientos' ? $detalle->cantidad : $detalle->area }}</td>
                        <td style="border: 1px solid #000; padding: 8px;">${{ $detalle->precio_unitario_format }}</td>
            ...
            <tr>
                ...
                <td
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                        ...
                        <tr>
                            <td style="text-align: right; padding: 8px;">DESCUENTO% {{ $proforma->descuento }}</td>
                            <td
        ...
        @if ($tipo == 'adecentamientos')
            ...
            <div style="background-color: #f2f2f2; padding: 15px; margin-top: 30px; border: 1px solid #ddd;">
                ...
                <p style="margin: 5px 0; font-size: 13px;"><strong>Validez de la proforma:</strong>
                    {{ $proforma->validez }}</p>
                <p style="margin: 5px 0; font-size: 13px;"><strong>Forma de pago:</strong> {{ $proforma->forma_pago }}
                </p>
                <p style="margin: 5px 0; font-size: 13px;"><strong>Tiempo de entrega:</strong>
                    {{ $proforma->plazo_entrega }}</p>
                <p style="margin: 5px 0; font-size: 13px;"><strong>Email:</strong> {{ $empresa[3]->detalle }}</p>
                <p style="margin: 5px 0; font-size: 13px;"><strong>Telf.:</strong> {{ $empresa[2]->detalle }}</p>
                <p style="margin: 5px 0; font-size: 13px;"><strong>Dir.:</strong> {{ $empresa[1]->detalle }}</p>
            </div>
        @endif
