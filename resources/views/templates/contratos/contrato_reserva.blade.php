<!DOCTYPE html>
<html lang="es">

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title>Contrato de Reserva</title>

</head>

<body
    style="font-family: 'Times New Roman', Times, serif; font-size: 12pt; line-height: 1.6; color: #333; background-color: #ffffff; margin: 0; padding: 0;">

    <div
        style="max-width: 800px; margin: 20px auto; border: 1px solid #ddd; box-shadow: 0 0 10px rgba(0,0,0,0.05); padding: 40px;">
        <!-- ======================= ENCABEZADO CON TABLA ======================= -->
        <table style="width: 100%; border-collapse: collapse; border-bottom: 2px solid #eeeeee; padding-bottom: 20px;">
            <tbody>
                <tr>
                    <td style="width: 50%; vertical-align: middle;">
                        <img src="{{ asset('images/prime_doc.png') }}" alt="Logotipo"
                            style="max-height: 70px; width: auto;">
                    </td>
                    <td
                        style="width: 50%; vertical-align: middle; text-align: right; font-family: Arial, sans-serif; font-size: 10pt; color: #666;">
                        <p style="margin: 0; line-height: 1.4;"><strong style="font-weight: bold;">CONSTRUCCIONES PRIME
                                JP S.A.</strong></p>
                        <p style="margin: 0; line-height: 1.4;">Av. 6 de noviembre y Augusto Gachet, sector El Colono
                        </p>
                        <p style="margin: 0; line-height: 1.4;">Santo Domingo, Ecuador</p>
                        <p style="margin: 0; line-height: 1.4;">Telf.: 0997457053</p>
                    </td>
                </tr>
            </tbody>
        </table>

        <!-- ======================= CUERPO DEL CONTRATO ======================= -->
        <div style="padding-top: 20px;">
            <h1 style="text-align: center; font-size: 18pt; margin-bottom: 5px; text-transform: uppercase;">Contrato de
                Reserva</h1>
            <h2 style="text-align: center; font-size: 14pt; font-weight: bold; color: #555; margin-bottom: 30px;">
                MANZANA # UNIDAD #</h2>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                PRIMERA: COMPARECIENTES</h3>
            <p style="text-align: justify; margin-bottom: 15px;">Comparecen al otorgamiento y suscripción de este
                contrato de reserva de compra de bienes inmuebles:</p>
            <ol style="padding-left: 40px;">
                <li style="margin-bottom: 10px; text-align: justify;">Por una parte: la compañía <strong
                        style="font-weight: bold;">CONSTRUCCIONES PRIME JP S.A.</strong> legalmente representada por el
                    Ing. John Patricio Narváez Abad en su calidad de Gerente General; parte a la cual se le denominará
                    la <strong style="font-weight: bold;">PROMOTORA</strong>; Y,</li>
                <li style="margin-bottom: 10px; text-align: justify;">Por otra parte, por sus propios derechos del señor
                    <strong style="font-weight: bold;">Velastegui Zambrano Fred Andrés</strong> con cédula de identidad
                    N° 171362226-2, parte a la cual se le denominará <strong style="font-weight: bold;">FUTURO
                        COMPRADOR</strong>. Los comparecientes son mayores de edad.
                </li>
            </ol>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                SEGUNDA: ANTECEDENTES</h3>
            <ol style="padding-left: 40px;">
                <li style="margin-bottom: 10px; text-align: justify;"><strong
                        style="font-weight: bold;">PROMOCIÓN.</strong> – La compañía CONSTRUCCIONES PRIME JP S.A. se
                    encuentra ejecutando obras de construcción de viviendas en el proyecto inmobiliario “RESIDENCIA
                    CONSTRUCCIONES PRIMEJP S.A.” dentro de la Urbanización Ciudad Celeste, al cual se le denominará en
                    adelante “EL PROYECTO”. – En razón de que CONSTRUCCIONES PRIME JP S.A. está promocionando la venta
                    de los bienes inmuebles que conformarán el proyecto inmobiliario “RESIDENCIA CONSTRUCCIONES PRIMEJP
                    S.A.”, en seguridad del Futuro Comprador y de los futuros compradores de unidades, la Promotora se
                    encuentra al momento suscribiendo contratos de reserva de inmuebles con los interesados en adquirir
                    los mismos.</li>
                <li style="margin-bottom: 10px; text-align: justify;"><strong style="font-weight: bold;">PERMISO DE
                        CONSTRUCCIÓN Y APROBACIÓN DE PLANOS.</strong> - Mediante el trámite #33825, Informe N.
                    PAGO-GADMSD-SGT-2024-1364-P se concede el PERMISO DE CONSTRUCCIÓN; y mediante el trámite #33825,
                    Informe N. PAGO-GADMSD-SGT-P-1364-2024 se concede la APROBACIÓN DE PLANOS del predio con dirección
                    CLUB PRIVADO CIUDAD CELESTE, manzana 2, lotes n. 8-9-10-11-12-13-14, Unidades de vivienda del 1 al
                    13, en el cantón Santo Domingo.</li>
                <li style="margin-bottom: 10px; text-align: justify;"><strong style="font-weight: bold;">EL FUTURO
                        COMPRADOR DECLARA</strong> conocer y estar plenamente conforme con la distribución
                    arquitectónica del bien inmueble, diseño y más detalles que constan en los planos del mismo.
                    Adicionalmente se realiza una propuesta de modificación del diseño interior de la cocina el mismo
                    que fue solicitado y aprobado.</li>
            </ol>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                TERCERA: RESERVA DEL INMUEBLE</h3>
            <p style="text-align: justify; margin-bottom: 15px;">Con los antecedentes expuestos, la compañía
                CONSTRUCCIONES PRIMEJP S.A. reserva a favor del señor Velastegui Zambrano Fred Andrés con cédula de
                identidad, quien declara aceptar la unidad 3 mz 2 que está disponible, ubicada dentro del CLUB PRIVADO
                CIUDAD CELESTE, de la ciudad de Santo Domingo, cantón Santo Domingo, de la provincia de Santo Domingo de
                los Tsáchilas, con una superficie de construcción de 116 metros cuadrados.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                CUARTA: PRECIO Y FORMA DE PAGO</h3>
            <p style="text-align: justify; margin-bottom: 15px;">El precio de venta que de común acuerdo han establecido
                las partes por el bien inmueble es de <strong style="font-weight: bold;">$90,000.00 (Noventa mil con
                    00/100 dólares americanos)</strong>, que será pagado por el futuro comprador a favor de futuro
                vendedor, de la siguiente manera:</p>
            <p style="text-align: justify; margin-bottom: 15px;">La cantidad que corresponda al 5%, <strong
                    style="font-weight: bold;">Cuatro mil quinientos dólares ($4,500.00)</strong> del valor pactado por
                el bien inmueble a la firma del contrato como entrada, para lo cual realiza un primer abono como reserva
                de <strong style="font-weight: bold;">$500.00 americanos</strong> mediante transferencia.</p>
            <p style="text-align: justify; margin-bottom: 15px;">La diferencia del valor del bien inmueble será
                cancelada mediante crédito bancario.</p>
            <p style="text-align: justify; margin-bottom: 15px;">Los valores deberán ser depositados exclusivamente por
                el titular del crédito (no por terceras personas) mediante depósito o transferencia bancaria a la cuenta
                corriente de la compañía CONSTRUCCIONES PRIME JP SA, del Banco Procredit N. 019037868916.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                QUINTA: CELEBRACIÓN DEL CONTRATO DE COMPRAVENTA Y ENTREGA DE LOS BIENES RESERVADOS</h3>
            <p style="text-align: justify; margin-bottom: 15px;">La PROMOTORA, se compromete en entregar el bien
                inmueble de acuerdo a la planificación y avance del proyecto, con todas las especificaciones
                arquitectónicas y estructurales en el Anexo 1 FICHA TÉCNICA. Además de la documentación del bien
                inmueble tales como, certificados, pagos de impuestos al día, y los que fuesen necesarios para realizar
                el contrato definitivo de compraventa.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                SEXTA: MULTA</h3>
            <p style="text-align: justify; margin-bottom: 15px;">En caso de que el futuro comprador por cualquier razón
                o circunstancia desista del cumplimiento del presente contrato o se negare a suscribir la compraventa
                definitiva o se hallare en mora por más de 72 días en el pago de cualquiera de las cuotas pactadas o en
                caso de no pagarse el saldo del precio en la fecha señalada en este instrumento, la Promotora se
                encuentra facultada expresamente para multar con el 100% del valor estipulado como reserva.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                SÉPTIMA: OTROS ACUERDOS</h3>
            <ol type="A" style="padding-left: 40px;">
                <li style="margin-bottom: 10px; text-align: justify;">En garantía de que el conjunto conserve siempre el
                    alto nivel inicial, el Futuro Comprador aceptan y se someten expresa e irrevocablemente a las
                    disposiciones que tenga la normativa arquitectónica aprobada, especialmente al tipo de
                    construcciones y al uso del espacio privativo de los futuros propietarios.</li>
                <li style="margin-bottom: 10px; text-align: justify;">EL Futuro Comprador declara que el bien inmueble
                    que adquirirá en virtud del presente instrumento, lo destinará exclusivamente a vivienda.</li>
                <li style="margin-bottom: 10px; text-align: justify;">Los gastos de guardianía y de mantenimiento de las
                    áreas comunales de cuya administración se encargarán que la asamblea de propietarios designe, serán
                    pagados por el Futuro Comprador desde el instante que se haya realizado el documento de compraventa
                    definitivo.</li>
                <li style="margin-bottom: 10px; text-align: justify;">El Futuro Comprador se obliga a cancelar todos los
                    gastos legales y notariales relativos a la celebración, otorgamiento, inscripción y catastro del
                    contrato de reserva y compraventa a su favor, incluyendo los impuestos municipales y los generados
                    por la transferencia de dominio.</li>
            </ol>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                OCTAVA: RESOLUCIÓN</h3>
            <p style="text-align: justify; margin-bottom: 15px;">Este contrato se resolverá de pleno derecho, sin
                intervención judicial y con renuncia del futuro comprador al derecho señalado en el artículo 1.819 del
                Código Civil, cuando la Promotora considere que no haya cumplido oportunamente con cualquiera de las
                obligaciones aquí contraídas. Si la Promotora ejerciere los derechos antes mencionados, quede
                expresamente autorizada por el Futuro Comprador para que pueda disponer libremente del inmueble, obras a
                construirse o en construcción y demás áreas comunes relativas al inmueble que es objeto del presente
                contrato.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                NOVENA: JURISDICCIÓN Y COMPETENCIA</h3>
            <p style="text-align: justify; margin-bottom: 15px;">Los compradores aceptan el presente contrato en todas
                sus partes y acuerdan que para cualquier diferencia, controversia o reclamación que se derive o esté
                relacionada con la ejecución del presente contrato o con el cumplimiento del mismo, que no pueda ser
                directamente solucionado entre ellas, será resulta por mediación en un Centro de Mediación de la ciudad
                de Santo Domingo.</p>

            <h3
                style="font-size: 12pt; text-transform: uppercase; border-bottom: 1px solid #eee; padding-bottom: 5px; margin-top: 30px;">
                DÉCIMA: NOTIFICACIONES</h3>
            <p style="text-align: justify; margin-bottom: 15px;">Para la recepción de cualquier tipo de notificación,
                las partes señalan como lugar de notificaciones las siguientes:<br>
                <strong style="font-weight: bold;">CONSTRUCTORA CONSTRUCCIONES PRIME JP S.A.</strong><br>
                Av. 6 de noviembre y Augusto Gachet, sector El Colono<br>
                c.primejp@gmail.com<br>
                Telf.: 0997457053<br>
                Santo Domingo
            </p>

            <p style="text-align: justify; margin-bottom: 15px;">El presente contrato ha sido negociado y acordado en su
                alcance y contenido por las partes contratantes, en virtud de lo cual de mutuo consentimiento lo aceptan
                en forma total y en constancia de lo acordado, los comparecientes suscriben el presente documento por
                triplicado, en la ciudad de Santo Domingo, al 22 de abril del 2025.</p>
        </div>

        <!-- ======================= SECCIÓN DE FIRMAS CON TABLA ======================= -->
        <table style="width: 100%; border-collapse: collapse; margin-top: 80px;">
            <tbody>
                <tr>
                    <td style="width: 50%; text-align: center; vertical-align: top; padding: 0 10px;">
                        <p
                            style="border-top: 1px solid #333; padding-top: 10px; margin-top: 60px; margin-bottom: 5px; font-size: 11pt; text-align: center;">
                            Ing. John Patricio Narváez Abad</p>
                        <p style="margin: 5px 0; font-size: 11pt; text-align: center;"><strong
                                style="font-weight: bold;">GERENTE GENERAL</strong></p>
                        <p style="margin: 5px 0; font-size: 11pt; text-align: center;">CONSTRUCCIONES PRIMEJP S.A.</p>
                        <p style="margin: 5px 0; font-size: 11pt; text-align: center;"><strong
                                style="font-weight: bold;">PROMOTORA</strong></p>
                    </td>
                    <td style="width: 50%; text-align: center; vertical-align: top; padding: 0 10px;">
                        <p
                            style="border-top: 1px solid #333; padding-top: 10px; margin-top: 60px; margin-bottom: 5px; font-size: 11pt; text-align: center;">
                            Sr. Velastegui Zambrano Fred Andrés</p>
                        <p style="margin: 5px 0; font-size: 11pt; text-align: center;"><strong
                                style="font-weight: bold;">CI: 171362226-2</strong></p>
                        <p style="margin: 5px 0; font-size: 11pt; text-align: center;"><strong
                                style="font-weight: bold;">COMPRADOR</strong></p>
                    </td>
                </tr>
            </tbody>
        </table>

    </div>
</body>

</html>
